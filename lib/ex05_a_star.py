...
def kinematicSimulationNode(currentNode, motionCommand, mapParameters, simulationLength=4, step=0.8):
    ...
    angle = rsCurve.pi_2_pi(
        currentNode.traj[-1][2] + motionCommand[1] * step / Car.wheelBase * math.tan(motionCommand[0]))
    ...
    for i in range(int((simulationLength / step)) - 1):
        traj.append([traj[i][0] + motionCommand[1] * step * math.cos(traj[i][2]),
                     ...
                     rsCurve.pi_2_pi(
                         traj[i][2] + motionCommand[1] * step / Car.wheelBase * math.tan(motionCommand[0]))])
...
def holonomicCostsWithObstacles(goalNode, mapParameters):
    gridIndex = [round(goalNode.traj[-1][0] / mapParameters.xyResolution),
...
def map():
    ...
    for i in range(321):
        obstacleX.append(i)
        obstacleY.append(240)

    for i in range(241):
        obstacleX.append(320)
        obstacleY.append(i)

    #parking grid
    for i in range(51):
        obstacleX.append(200)
        obstacleY.append(i)
    ...
    for i in range(51):
        obstacleX.append(150)
        obstacleY.append(i)

    for i in range(51):
        obstacleX.append(250)
        obstacleY.append(i)
...
def run(s, g, mapParameters, plt):
    ...
    costQueue[index(startNode)] = max(startNode.cost, Cost.hybridCost * holonomicHeuristics[startNode.gridIndex[0]][
        startNode.gridIndex[1]])
    counter = 0
        ...
        for i in range(len(motionCommand)):
            ...
            if simulatedNodeIndex not in closedSet:
                ...
                if simulatedNodeIndex not in openSet:
                    ...
                    costQueue[simulatedNodeIndex] = max(simulatedNode.cost, Cost.hybridCost *
                                                        holonomicHeuristics[simulatedNode.gridIndex[0]][
                                                            simulatedNode.gridIndex[1]])
                else:
...
def main():
    ...
    s = [20, 75, np.deg2rad(0)]
    g = [225, 25, np.deg2rad(90)]
    ...
    opt_path = [[x[i], y[i], yaw[i]] for i in range(len(x))]
    print(opt_path)
